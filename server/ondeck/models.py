...
#  u = User.objects.create_user(username="ben", email="ben@comediadesign.com"); w = Workspace.objects.create(name="Comedia", slug="comedia", owner=u); b = Board.objects.create(name="ondeck", slug="ondeck", workspace=w)
...
class Ticket(models.Model):
    ...
    @property
    def key(self):
        return "{}-{}".format(self.board.key, self.index)


@receiver(pre_save, sender=Ticket)
def assign_last_key(sender, instance, **kwargs):
    instance.index = instance.board.last_index


@receiver(post_save, sender=Ticket)
def save_new_last_index(sender, instance, **kwargs):
    board = instance.board
    board.last_index = instance.index + 1
    board.save()
